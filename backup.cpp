...
std::string ListaDuplamenteEncadeada::recuperar(int i)
{
   ...
   if(i == 0){return this -> cabeca->getProximo()->getValor();}
   if(i == quantidade){return this -> cauda->getAnterior()->getValor();}

    No<std::string>* aux = cabeca; // Pular a cabeça
	for(int j = 0 ; j < i ; j++){
		aux = aux -> getProximo();
	}
...
{
	No<std::string>* aux = cabeca -> getProximo();
	int i = 0;
	while(aux != cauda){
		if(aux -> getValor() == s){
			return i+1;
...
{   
	...
	novo -> setAnterior(this -> cabeca); //ou this -> head
	novo -> setProximo(aux);
	aux -> setAnterior(novo);
	this -> cabeca -> setProximo(novo); //ou this -> head
...
{    
	...
	novo -> setProximo(this -> cauda); //ou this->cauda
	aux -> setProximo(novo);
	this -> cauda -> setAnterior(novo); //ou this-> cauda
...
{    
	if(cauda -> getAnterior() == cabeca){return "";}

	auto aux = cauda->getAnterior();

	if(aux == cabeca){return "";}
